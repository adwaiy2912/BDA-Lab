...
spark = SparkSession.builder.appName("EmployeeChurnPrediction").getOrCreate()

# a) Load and preprocess employee data from HDFS
# Replace 'hdfs_path' with your actual path
data = spark.read.csv("/user/220968310/week10/employee_attrition.csv", header=True, inferSchema=True)
...
categorical_columns = ['Department', 'JobRole', 'Education', 'OverTime']
...
feature_columns = ['Age', 'YearsAtCompany', 'MonthlyIncome', 'WorkLifeBalance', 'JobSatisfaction', 'Department_index', 'JobRole_index', 'Education_index', 'OverTime_index']
...
pandas_df = data.select("Age", "YearsAtCompany", "MonthlyIncome", "WorkLifeBalance", "JobSatisfaction").toPandas()
...
# c) Build a classification model to predict employee churn (Attrition)
# Split the data into train and test
train_data, test_data = data.randomSplit([0.8, 0.2], seed=123)

# Logistic Regression for classification
lr = LogisticRegression(featuresCol='scaled_features', labelCol='Attrition')
...
binary_evaluator = BinaryClassificationEvaluator(labelCol="Attrition")
classification_auc = binary_evaluator.evaluate(lr_predictions)
print(f"Classification AUC: {classification_auc}")

# d) Train a regression model to predict employee salaries (MonthlyIncome)
# For salary prediction, we use Linear Regression
lr_salary = LinearRegression(featuresCol='scaled_features', labelCol='MonthlyIncome')
...
regression_evaluator = RegressionEvaluator(labelCol="MonthlyIncome")
...
plt.scatter(salary_predictions.select("MonthlyIncome").toPandas(), salary_predictions.select("prediction").toPandas(), alpha=0.5)
plt.xlabel("Actual MonthlyIncome")
plt.ylabel("Predicted MonthlyIncome")
plt.title("Actual vs Predicted MonthlyIncome (Regression)")
plt.show()
